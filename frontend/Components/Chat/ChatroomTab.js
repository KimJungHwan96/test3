...
export default class ChatroomTab extends Component {
    ...
    createRoom = (inputText) => { // 키워드를 입력하여 버튼을 누르면 서버에 방을 만들고 방 번호를 출력해줌.
        ...
        .then(responseJson => {
            console.log(responseJson);
            this.suggestChatRoom(responseJson.chatroom_id, responseJson.name, responseJson.interest);
        })
    };

    insertChatRoom = (chatroom_id, name, interest) => { // 여기에다 ROOMtitle 이냐 RoomID냐에 따라 push 를 다르게 지정
        this.array.push({
            title : name,
            roomID: chatroom_id,
            interest: interest
    ...
    suggestChatRoom = (chatroom_id, interest) => {
